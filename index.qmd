...
pdtg_ripple$ripple_score <- lambda_ripple *(pdtg_ripple$values / (log(pdtg_ripple$group + 1)))
pdtg_ripple <- pdtg_ripple |>
  mutate(ripple_score = if_else(is.infinite(ripple_score), values, ripple_score))
...
rep_frame <- data.frame(
  group = rep(pdtg_ripple$group, times = pdtg_ripple$adj_score),
  group_name = rep(pdtg_ripple$group_name, pdtg_ripple$adj_score)
  )

spiral_frame <- vogel_spiral(sum(pdtg_ripple$adj_score))
...
arch_title <- data.frame(
  x = 1,
  y = 1,
  label = glue("Composite Impact Score <b>{pdtg_overall_score}</b>")
)
arch_line <<- ggplot(
  arch_title,
  aes(
...
ripple_title <- data.frame(
  x = 1,
  y = 1,
  label = glue("Ripple Effect Score <b>{total_ripple_score}</b>")
)
ripple_line <<- ggplot(
  ripple_title,
  aes(
...
total_indicator_score <- category_scores |>
  filter(project_name == "PD @ TG") |>
  tally(category_impact_number) |>
    pull(n)

indicator_title <- data.frame(
  x = 1,
  y = 1,
  label = glue("Direct Indicator Score <b>{total_indicator_score}</b>"))
